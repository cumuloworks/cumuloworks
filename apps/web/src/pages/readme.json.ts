...
/* Repository root (where README.md lives): apps/web/src/pages -> 5 up. */
...
function socialBlock(): string {
  ...
  return items
    .map(
      (s) =>
        `<a href="${s.url}" title="${s.name}"><img src="apps/web/public/icons/social/${s.id}.svg" width="24" alt="${s.name}" /></a>`
    )
    .join('&nbsp;&nbsp;\n');
}
